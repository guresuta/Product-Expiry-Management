(function () {
  ...
  const dictionaries = {
    en: {
      ...
      "相機只用於條碼掃描。掃描時影像會在裝置本機交給瀏覽器或 WebView 的條碼辨識能力處理。": "The camera is used only for barcode scanning. Images are processed locally by the browser or WebView barcode scanner.",
    ...
    ja: {
      ...
      "相機權限": "カメラ権限",
      "相機只用於條碼掃描。掃描時影像會在裝置本機交給瀏覽器或 WebView 的條碼辨識能力處理。": "カメラはバーコードスキャンのみに使用します。スキャン時の画像は、端末内でブラウザまたは WebView のバーコード認識機能により処理されます。",
  ...
  function shouldSkipI18n(node) {
    ...
    return !!element?.closest?.("[data-i18n-skip='true']");
  }
