(function () {
  ...
  const dictionaries = {
    en: {
      ...
      "請先使用「備份資料 JSON 」建立備份，後續更新的版本只會使用本機 IndexedDB做為資料儲存方式 ，不會再讀取或自動將先前指定的本機檔案位置匯入資料庫。若曾使用「本機檔案位置」，請在下一個程式版本更新前匯出JSON ，並在更新後用「還原資料 JSON 」恢復資料。": "Create a backup first with Back Up JSON.\n\nFuture versions will use only IndexedDB on this device and will no longer read from or automatically import a previously selected local file.\n\nIf you used Local File, export JSON before updating to the next app version, then restore your data with Restore JSON Backup after updating.",
...
"v2.0.8更新內容": "v2.0.8 Release Notes",
    ...
    ja: {
      ...
      "請先使用「備份資料 JSON 」建立備份，後續更新的版本只會使用本機 IndexedDB做為資料儲存方式 ，不會再讀取或自動將先前指定的本機檔案位置匯入資料庫。若曾使用「本機檔案位置」，請在下一個程式版本更新前匯出JSON ，並在更新後用「還原資料 JSON 」恢復資料。": "先に「JSON バックアップ」でバックアップを作成してください。\n\n今後のバージョンでは、端末内の IndexedDB のみを保存先として使用し、以前に指定したローカルファイルを読み込んだり、データベースへ自動的にインポートしたりしなくなります。\n\nローカルファイルを使用していた場合は、次のアプリ更新前に JSON を書き出し、更新後に「JSON バックアップを復元」でデータを復元してください。",
      "了解並繼續": "確認して続ける",
      "建議備份資料": "バックアップ推奨",
