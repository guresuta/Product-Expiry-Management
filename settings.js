(function () {
  ...
  let suppressDeleteCategoryBackdropUntil = 0;
  ...
  function openDeleteCategoryModal(category, options = {}) {
    pendingDeleteCategory = String(category || "").trim();
    if (!pendingDeleteCategory) {
      return;
    }
    if (options.suppressBackdropClick) {
      suppressDeleteCategoryBackdropUntil = Date.now() + 650;
    }
  ...
  function requestDeleteCategory(category, options = {}) {
    if (!category) {
      return;
    }
    if (state.categories.length <= 1) {
    ...
    openDeleteCategoryModal(category, options);
  }
  ...
  function wireEvents() {
    ...
    if (ui.deleteCategoryModal) {
      ui.deleteCategoryModal.addEventListener("click", (event) => {
        ...
        if (Date.now() < suppressDeleteCategoryBackdropUntil) {
          event.preventDefault();
          event.stopPropagation();
          return;
        }
    ...
    ui.categoryList.addEventListener("pointerup", (event) => {
      ...
      const category = button.getAttribute("data-delete-category");
      requestDeleteCategory(category, { suppressBackdropClick: true });
    });
